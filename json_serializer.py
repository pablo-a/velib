...
class JSONEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        ...
        # Si l'objet est de type Decimal (vient du type Decimal de MySQL)
        # on le convertit en float (type natif à Python).
...
class JSONDecoder(json.JSONDecoder):
    ...
    def decode_on_match(self, obj):
        ...
        match = re.search(r'\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}', unicode(obj))
...
# raccourcis pour loader et dumper le json
def json_dumps(data):
